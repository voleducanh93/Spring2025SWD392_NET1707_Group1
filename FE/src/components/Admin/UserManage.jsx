...
const UserManage = () => {
    ...
    { title: "Ảnh", dataIndex: "imageUrl", key: "imageUrl", render: (imageUrl) => (imageUrl ? <Avatar src={imageUrl} /> : <Avatar>👤</Avatar>) },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm title="Bạn có chắc muốn xóa người dùng này?" onConfirm={() => handleDelete(record.id)} okText="Có" cancelText="Không">
  ...
  const userDetailFields = [
    ...
    { name: "isActive", label: "Trạng Thái", render: (isActive) => (isActive ? "✅ Hoạt động" : "⛔ Bị khóa") },
    { name: "email", label: "Email" },
    { name: "imageUrl", label: "Ảnh", render: (url) => (url ? <Avatar src={url} size={64} /> : "Không có ảnh") },
  ...
  const userFields = [
    { name: "fullName", label: "Họ & Tên", rules: [{ required: true, message: "Vui lòng nhập họ và tên!" }] },
    { name: "userName", label: "Tên Đăng Nhập", rules: [{ required: true, message: "Vui lòng nhập tên đăng nhập!" }] },
    { name: "email", label: "Email", rules: [{ required: true, message: "Vui lòng nhập email!" }] },
    { name: "phoneNumber", label: "Số điện thoại", rules: [{ required: true, message: "Vui lòng nhập số điện thoại!" }] },
...
const showEditModal = (record) => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <Button type="primary" onClick={showCreateModal}>➕ Thêm Người Dùng</Button>
      <CustomTable columns={columns} dataSource={users} loading={isLoading} rowKey="id" />
      
      {/* Modal Create & Update */}
      <CustomModal visible={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleSubmit} formFields={userFields} form={form} />
      
      {/* Modal Chi tiết */}
      <DetailModal visible={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} data={selectedUser} fields={userDetailFields} />
    </div>
  );
};
