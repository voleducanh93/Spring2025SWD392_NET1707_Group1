...
const ComboManagement = () => {
  const { combos, isLoading, addCombo, editCombo, removeCombo } = useComboVaccine();
  console.log();
    ...
    form.setFieldsValue(
      combo
        ? {
            ...
            vaccineIds: combo.vaccines ? combo.vaccines.map((vaccine) => vaccine.vaccineId) : [],
          }
        : {
            comboName: "",
            description: "",
  ...
  const handleOk = async () => {
    try {
        ...
        if (editingCombo) {
            const existingVaccineIds = editingCombo.vaccines.map(v => v.vaccineId); // Lấy danh sách vaccine cũ
            const newVaccineIds = comboData.vaccineIds || []; // Lấy danh sách vaccine mới

            // Kiểm tra xem có thay đổi vaccine không
            const isVaccineChanged = JSON.stringify(existingVaccineIds.sort()) !== JSON.stringify(newVaccineIds.sort());

            // Nếu không thay đổi vaccine, gửi danh sách cũ
            const updatedVaccineIds = isVaccineChanged ? newVaccineIds : existingVaccineIds;
  ...
  const columns = [
    ...
    { title: "Tổng giá", dataIndex: "totalPrice", key: "totalPrice", render: (text) => `${text.toLocaleString()} VND` },
    {
      title: "Trạng thái",
      dataIndex: "isActive",
      key: "isActive",
...
>
  ...
  return (
    ...
    >
      ...
      <ComboModal isOpen={isModalOpen} handleClose={() => setIsModalOpen(false)} handleOk={handleOk} form={form} editingCombo={editingCombo} />

      {/* Modal Chi Tiết Combo */}
      <ComboDetailModal isOpen={isDetailModalOpen} handleClose={handleDetailCancel} selectedCombo={selectedCombo} />
    </div>
  );
};
